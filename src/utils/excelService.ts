...
export const ExcelService = {
    downloadTemplate(type: 'shipments' | 'consolidations') {
        ...
        if (type === 'shipments') {
            headers = [
                'tracking_number', 'origin_port', 'origin_country', 'destination_port',
                'destination_country', 'carrier_name', 'cargo_type', 'cargo_weight',
                'cargo_volume', 'cargo_packages', 'departure_date', 'arrival_estimated_date'
            ];
            example = [
                'TRK123456', 'Shanghai', 'China', 'Dakar',
                'Senegal', 'Maersk', 'General', 1500,
                12.5, 50, '2025-01-01', '2025-02-15'
            ];
            sheetName = 'Expéditions';
        } else {
            headers = [
                'type', 'origin_port', 'destination_port', 'transport_mode',
                'departure_date', 'arrival_date', 'total_capacity_cbm',
                'price_per_cbm', 'min_cbm', 'status'
            ];
            example = [
                'sea', 'Ningbo', 'Abidjan', 'Container',
                '2025-03-01', '2025-04-01', 60,
                120, 1, 'planned'
            ];
            sheetName = 'Groupages';
        }
        ...
        console.log(`Generated Excel Base64 size: ${wbout.length} chars`);
