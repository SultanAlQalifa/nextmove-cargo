...
export const generateInvoice = (data: InvoiceData) => {
    ...
    doc.text(`Route: ${shipment.origin_country} -> ${shipment.destination_country}`, 140, 61);
...
export const generateWaybill = (shipment: ShipmentData) => {
    ...
    doc.setFontSize(12);
    doc.text(shipment.forwarder.company_name || shipment.forwarder.full_name || 'Forwarder', 15, 68);
    doc.setFontSize(10);
    ...
    doc.text('2. Carrier liability is limited by international conventions.', 15, 160);
...
export const generateSubscriptionContract = (data: ContractData) => {
    ...
    doc.text(`Réf: CTR-${Math.floor(Math.random() * 10000)}`, 105, 35, { align: 'center' });
    ...
    doc.setFontSize(10);
    doc.text('Le présent contrat a pour objet la souscription aux services de la plateforme NextMove Cargo.', 15, 146);

    doc.setFontSize(12);
    doc.text('Article 2 : Détails de l\'abonnement', 15, 156);
    doc.setFontSize(10);
    doc.text(`Plan souscrit : ${plan.name}`, 15, 162);
    doc.text(`Prix : ${plan.price} ${plan.currency} / ${plan.billing_cycle === 'monthly' ? 'Mois' : 'An'}`, 15, 167);

    doc.setFontSize(12);
    doc.text('Article 3 : Durée et Résiliation', 15, 180);
    doc.setFontSize(10);
    doc.text('L\'abonnement est conclu pour une durée indéterminée avec tacite reconduction.', 15, 186);
...
export const generateSubscriptionInvoice = (data: ContractData) => {
    ...
    autoTable(doc, {
        ...
        body: [
            [`Abonnement ${plan.name} (${plan.billing_cycle === 'monthly' ? 'Mensuel' : 'Annuel'})`, `${baseAmount.toLocaleString()} ${plan.currency}`],
            ['Frais de transaction (1%)', `${fees.toLocaleString()} ${plan.currency}`],
    ...
    doc.setFontSize(12);
    doc.setTextColor(0);
    doc.text(`Total à payer: ${totalAmount.toLocaleString()} ${plan.currency}`, 140, finalY + 20);
