 ...
 */
export function formatLargeNumber(amountUSD: number, targetCurrency: string): string {
    ...
    if (convertedAmount >= 1000000000) {
        ...
        return targetCurrency === 'XOF' ? `${value}Mrd ${symbol}` : `${symbol}${value}B`;
    }
    if (convertedAmount >= 1000000) {
        const value = (convertedAmount / 1000000).toFixed(0);
        return targetCurrency === 'XOF' ? `${value}M ${symbol}` : `${symbol}${value}M`;
    }
    if (convertedAmount >= 1000) {
        const value = (convertedAmount / 1000).toFixed(0);
        return targetCurrency === 'XOF' ? `${value}K ${symbol}` : `${symbol}${value}K`;
    }
    return targetCurrency === 'XOF' ? `${convertedAmount} ${symbol}` : `${symbol}${convertedAmount}`;
}
