import { X, FileText, Calendar, User, CheckCircle, XCircle, Package, Truck, Download } from 'lucide-react';
...
export default function PODDetailsModal({
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'verified':
                return <span className="px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800 flex items-center gap-1"><CheckCircle className="w-3 h-3" /> Vérifié</span>;
            case 'rejected':
                ...
                <div className="bg-white rounded-2xl w-full max-w-2xl shadow-xl animate-in zoom-in-95 duration-200 overflow-hidden flex flex-col max-h-[90vh]">
                    <div className="flex justify-between items-center p-6 border-b border-gray-100">
                        <div>
                            ...
                            <p className="text-sm text-gray-500 mt-1">Soumis le {new Date(pod.submitted_at).toLocaleDateString()}</p>
                        </div>
                    ...
                    <div className="p-6 overflow-y-auto space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div className="bg-white border border-gray-100 rounded-xl p-4">
                                    <p className="font-medium text-gray-900">{pod.forwarder.name}</p>
                                    <p className="text-xs text-gray-500 mt-1">ID: {pod.forwarder.id}</p>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="bg-white border border-gray-100 rounded-xl p-4">
                                    ...
                                    <p className="text-xs text-gray-500 mt-1">ID: {pod.client.id}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            <h3 className="text-sm font-medium text-gray-900 mb-3 flex items-center gap-2">
                                <FileText className="w-4 h-4 text-gray-400" /> Documents Justificatifs
                            </h3>
                                    ...
                                    pod.documents.map((doc, index) => (
                                        ...
                                        >
                                            ...
                                            <div
                                                ...
                                                className="flex"
                                            >
                                                <a href={doc.url} download className="p-2 text-gray-400 hover:text-primary hover:bg-white rounded-lg transition-colors">
