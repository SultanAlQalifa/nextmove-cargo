...
export default function CreatePlanModal({
  ...
  const [newFeatureType, setNewFeatureType] = useState<"boolean" | "limit">(
    "boolean",
  );
  ...
  const handleSaveFeature = () => {
    ...
    const featureValue =
      newFeatureType === "limit" ? Number(newFeatureValue) || 0 : true;
      ...
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div
              className={`grid grid-cols-1 sm:grid-cols-12 gap-2 items-end p-3 rounded-xl transition-colors ${editingFeatureId ? "bg-primary/5 border border-primary/20" : "bg-gray-50"}`}
            >
              ...
              <div className="sm:col-span-3 space-y-1">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewFeatureType(e.target.value as "boolean" | "limit")
                  }
            ...
            <div className="space-y-2">
              {formData.features.map((feature) => (
                <div
                  key={feature.id}
                  className={`flex items-center justify-between p-3 bg-white border rounded-xl group transition-all ${editingFeatureId === feature.id ? "border-primary ring-1 ring-primary/20" : "border-gray-100 hover:border-primary/20"}`}
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`p-1.5 rounded-lg ${feature.type === "limit" ? "bg-blue-50 text-blue-600" : "bg-green-50 text-green-600"}`}
                    >
                      {feature.type === "limit" ? (
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
            <button
              type="button"
              onClick={onClose}
              className="px-6 py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-50 rounded-xl transition-colors min-h-[44px]"
            >
              Annuler
            </button>
            <button
              type="submit"
              disabled={loading}
              className="px-6 py-2.5 text-sm font-medium text-white bg-primary hover:bg-primary/90 rounded-xl shadow-lg shadow-primary/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed min-h-[44px]"
            >
              {loading
                ? "Enregistrement..."
                : isEditing
