import { X, FileText, Calendar, User, Wallet, CheckCircle, XCircle } from 'lucide-react';
...
export default function FundCallDetailsModal({
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'approved':
                return <span className="px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800 flex items-center gap-1"><CheckCircle className="w-3 h-3" /> Approuvé</span>;
            case 'rejected':
                return <span className="px-3 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800 flex items-center gap-1"><XCircle className="w-3 h-3" /> Rejeté</span>;
            case 'paid':
                return <span className="px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800 flex items-center gap-1"><Wallet className="w-3 h-3" /> Payé</span>;
            default:
                return <span className="px-3 py-1 rounded-full text-xs font-medium bg-orange-100 text-orange-800 flex items-center gap-1"><Calendar className="w-3 h-3" /> En attente</span>;
        }
    };
            ...
            <div className="bg-white rounded-2xl w-full max-w-2xl shadow-xl animate-in zoom-in-95 duration-200 overflow-hidden flex flex-col max-h-[90vh]">
                <div className="flex justify-between items-center p-6 border-b border-gray-100">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-1">Soumis le {new Date(fundCall.created_at).toLocaleDateString()}</p>
                    </div>
                ...
                <div className="p-6 overflow-y-auto space-y-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center bg-gray-50 p-4 rounded-xl">
                        ...
                        <div>
                            {getStatusBadge(fundCall.status)}
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="bg-white border border-gray-100 rounded-xl p-4">
                            <p className="font-medium text-gray-900">{fundCall.requester.name}</p>
                            <p className="text-sm text-gray-500">{fundCall.requester.email}</p>
                            <span className="inline-block mt-2 text-xs px-2 py-0.5 bg-gray-100 text-gray-600 rounded">
                                {fundCall.requester.role === 'forwarder' ? 'Transitaire' : 'Partenaire'}
                            </span>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="bg-white border border-gray-100 rounded-xl p-4">
                            <p className="text-sm text-gray-700 leading-relaxed">{fundCall.reason}</p>
                        </div>
                    </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {fundCall.attachments.length > 0 ? (
                                fundCall.attachments.map((doc, index) => (
                                    <div key={index} className="flex items-center gap-3 p-3 border border-gray-100 rounded-xl hover:bg-gray-50 transition-colors cursor-pointer group">
