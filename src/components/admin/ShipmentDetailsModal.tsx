import { X, Package, MapPin, Calendar, Truck, User, CheckCircle, Clock, AlertCircle } from 'lucide-react';
...
export default function ShipmentDetailsModal({ shipment, onClose, onTrack }: ShipmentDetailsModalProps) {
    const getStatusColor = (status: string) => {
        switch (status) {
            case 'Livré': return 'bg-green-100 text-green-700';
            case 'En transit': return 'bg-blue-100 text-blue-700';
    ...
    const getStatusIcon = (status: string) => {
        switch (status) {
            ...
            case 'En transit': return <Truck className="w-5 h-5" />;
        ...
        <div className="fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl animate-in fade-in zoom-in duration-200 overflow-hidden">
                ...
                <div className="flex justify-between items-center p-6 border-b border-gray-100 bg-gray-50/50">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="bg-white p-1 rounded-lg border border-gray-100 shadow-sm">
                            <QRCodeSVG value={`https://nextmove.com/track/${shipment.id}`} size={48} />
                        </div>
                        <button
                            onClick={onClose}
                ...
                <div className="p-6 space-y-8">
                    {/* Status Banner */}
                    <div className={`flex items-center gap-3 p-4 rounded-xl ${getStatusColor(shipment.status)} bg-opacity-50`}>
                        {getStatusIcon(shipment.status)}
                        <div>
                            <p className="font-medium">Statut: {shipment.status}</p>
                            <p className="text-xs opacity-80">Dernière mise à jour: {new Date(shipment.created_at).toLocaleDateString()}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between relative">
                        ...
                        <div className="bg-white pr-4">
                            <div className="flex items-center gap-2 text-gray-500 mb-1">
                                ...
                                <span className="text-xs font-medium uppercase tracking-wider">Origine</span>
                            </div>
                        ...
                        <div className="bg-white pl-4 text-right">
                            <div className="flex items-center gap-2 text-gray-500 mb-1 justify-end">
                                <span className="text-xs font-medium uppercase tracking-wider">Destination</span>
                                <MapPin className="w-4 h-4" />
                            </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-900">{new Date(shipment.created_at).toLocaleDateString()}</p>
                                </div>
                            </div>
                        </div>
                        <div className="space-y-4">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <p className="text-sm font-medium text-gray-500">Détails du colis</p>
                                    <p className="text-gray-900">{shipment.type} • {shipment.weight}</p>
                                </div>
                            </div>
                        </div>
                    </div>
