import { X, ShieldCheck, AlertTriangle, FileText, Check, MapPin, Mail, Phone, Calendar, Building2, Clock, Ban, PowerOff, PauseCircle, Trash2, Power, Edit, Star, TrendingUp } from 'lucide-react';
            ...
            <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-xl animate-in zoom-in-95 duration-200">
                ...
                <div className="sticky top-0 bg-white z-10 border-b border-gray-100 px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-gray-900">{forwarder.company_name}</h2>
                            <div className="flex items-center gap-3 text-sm text-gray-500">
                                <span>ID: {forwarder.id}</span>
                                <span>•</span>
                                <span>Inscrit le {new Date(forwarder.joined_at).toLocaleDateString()}</span>
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        {getStatusBadge(forwarder.verification_status, forwarder.account_status)}
                        <div className="bg-white p-1 rounded-lg border border-gray-100 shadow-sm">
                            <QRCodeSVG value={`https://nextmove.com/forwarder/${forwarder.id}`} size={40} />
                        </div>
                        <button
                            onClick={onClose}
                ...
                <div className="p-6 space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-4">
                            <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wider">Informations de Contact</h3>
                            <div className="space-y-3">
                                <div className="flex items-center gap-3 text-gray-600">
                                    <Mail className="w-5 h-5 text-gray-400" />
                                    <a href={`mailto:${forwarder.email}`} className="hover:text-primary transition-colors">{forwarder.email}</a>
                                </div>
                                <div className="flex items-center gap-3 text-gray-600">
                                    <Phone className="w-5 h-5 text-gray-400" />
                                    <a href={`tel:${forwarder.phone}`} className="hover:text-primary transition-colors">{forwarder.phone}</a>
                                </div>
                                <div className="flex items-center gap-3 text-gray-600">
                                    <MapPin className="w-5 h-5 text-gray-400" />
                                    <span>{forwarder.address}, {forwarder.country}</span>
                                </div>
                            </div>
                        </div>

                        <div className="space-y-4">
                            <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wider">Détails de l'Entreprise</h3>
                            <div className="bg-gray-50 rounded-xl p-4 space-y-3">
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-medium text-gray-900">Logistique Internationale</span>
                                </div>
                                <div className="flex justify-between">
                    ...
                    {/* Documents Section */}
                    <div>
                        <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">Documents Légaux</h3>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {forwarder.documents.map((doc) => (
                                <div key={doc.id} className="flex items-center p-4 border border-gray-200 rounded-xl hover:border-primary/50 hover:bg-primary/5 transition-all group cursor-pointer">
                                    ...
                                    <div className="ml-4 flex-1">
                                        ...
                                        <p className="text-xs text-gray-500 uppercase">{doc.type} • {doc.status}</p>
                                    </div>
                                    <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                if (doc.url === '#') {
                                                    toastError("Aperçu du document non disponible en mode démo");
                                                } else {
                    ...
                    <div className="bg-gray-50 -mx-6 -mb-6 p-6 border-t border-gray-100 flex flex-col gap-4 mt-8">
                        ...
                        {forwarder.verification_status === 'verified' && (
                            <div className="flex flex-wrap justify-end gap-3 pt-4 border-t border-gray-200">
                                ...
                                <button
                                    onClick={() => onToggleFeature && onToggleFeature(forwarder.id)}
                                ...
                                >
                                    <Star className={`w-4 h-4 ${forwarder.isFeatured ? 'fill-yellow-500 text-yellow-500' : ''}`} />
                                    {forwarder.isFeatured ? 'En vedette' : 'Mettre en vedette'}
                                </button>
                                <button
                                    onClick={() => onTogglePromote && onTogglePromote(forwarder.id)}
                                ...
                                >
                                    <TrendingUp className={`w-4 h-4 ${forwarder.isPromoted ? 'text-purple-600' : ''}`} />
