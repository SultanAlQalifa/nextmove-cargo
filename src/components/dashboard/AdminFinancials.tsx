...
export default function AdminFinancials() {
    ...
    const loadFinancials = async () => {
        ...
        // 2. Calculate stats
        const totalRevenue = payments.reduce((sum, p) => sum + (p.fee_platform || 0), 0);
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div className="bg-white overflow-hidden shadow rounded-lg p-5">
                    <div className="flex items-center">
                        ...
                        <div className="ml-5 w-0 flex-1">
                            <dl>
                                <dt className="text-sm font-medium text-gray-500 truncate">Escrow (Blocked)</dt>
                                <dd className="text-lg font-medium text-gray-900">€{stats.blockedFunds.toFixed(2)}</dd>
                            </dl>
                        </div>
                    </div>
                </div>
                ...
                <div className="border-t border-gray-200">
                    <ul className="divide-y divide-gray-200">
                      ...
                      ${payment.status === 'released' ? 'bg-green-100 text-green-800' :
                                                payment.status === 'blocked' ? 'bg-yellow-100 text-yellow-800' :
