      ...
      <div className="border-t border-gray-200">
        <ul role="list" className="divide-y divide-gray-200">
          {forwarders.map((forwarder) => {
            const details = forwarder.forwarder_details[0] || {
              verified: false,
              ...
              >
                <div>
                  <p className="text-sm font-medium text-primary truncate">
                    {forwarder.company_name || forwarder.email}
                  </p>
                  <p className="text-sm text-gray-500">{forwarder.full_name}</p>
                </div>
                <div className="flex items-center gap-4">
                  <span
                    className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${details.verified ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                  >
                    {details.verified ? "Verified" : "Unverified"}
                  </span>
                  <button
                    onClick={() =>
                      toggleVerification(forwarder.id, details.verified)
                    }
