...
import type { CreateRFQData, TransportMode, ServiceType } from '../../types/rfq';
...
export default function CreateRFQForm() {
    ...
    useEffect(() => {
        ...
        if (forwarderId || origin || destination || mode) {
            setFormData(prev => ({
                ...
                service_type: (searchParams.get('type') as ServiceType) || prev.service_type,
        ...
        if (location.state) {
            ...
            // Handle Calculator Prefill
                ...
                if (prefill.cargo_details) {
                    if (prefill.cargo_details.unit) setDimensionUnit(prefill.cargo_details.unit as LengthUnit);
                    if (prefill.cargo_details.length) setLength(prefill.cargo_details.length);
                    if (prefill.cargo_details.width) setWidth(prefill.cargo_details.width);
                    if (prefill.cargo_details.height) setHeight(prefill.cargo_details.height);
                }
                ...
                setFormData(prev => ({
                    ...
                    transport_mode: (prefill.transport_mode as TransportMode) || prev.transport_mode,
                    service_type: (prefill.service_type as ServiceType) || prev.service_type,
                    weight_kg: prefill.cargo_details?.weight ? parseFloat(prefill.cargo_details.weight) : undefined,
                    budget_amount: prefill.budget ? Math.round(prefill.budget) : undefined,
                    // Only set specific_forwarder_id if it's a valid UUID
                    specific_forwarder_id: (prefill.target_forwarder && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(prefill.target_forwarder))
    ...
    const handleSubmit = async (e: React.FormEvent | React.MouseEvent, publish: boolean = false) => {
        e.preventDefault();
        setError(null);
        ...
        if (formData.transport_mode === 'sea' && (!formData.volume_cbm || formData.volume_cbm <= 0)) {
            setError('Le volume (CBM) est requis pour le fret maritime. Veuillez entrer les dimensions.');
            return;
        }

        if (formData.transport_mode === 'air' && (!formData.weight_kg || formData.weight_kg <= 0)) {
    ...
    const standardDetails = getServiceDetails('standard', formData.transport_mode);
        ...
        return (
            <form ref={formRef} className="max-w-4xl mx-auto p-6">
                <div className="mb-8">
                    ...
                    <p className="text-gray-600">Confirmez les détails de votre expédition pour valider la demande.</p>
                </div>
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-white rounded-xl shadow-lg border-2 border-primary/10 overflow-hidden">
                            <div className="bg-primary/5 px-6 py-4 border-b border-primary/10 flex justify-between items-center">
                                ...
                                <span className="bg-primary text-white text-xs px-3 py-1 rounded-full font-medium">OFFICIEL</span>
                            </div>
                            <div className="p-6">
                                <div className="flex justify-between items-start mb-6">
                                    <div>
                                        <h3 className="text-xl font-bold text-gray-900">{quote.forwarder_name}</h3>
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className="text-2xl font-bold text-primary">
                                            {new Intl.NumberFormat(undefined, { style: 'currency', currency: formData.budget_currency || 'XOF', maximumFractionDigits: 0 }).format(quote.total_cost)}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 bg-gray-50 rounded-lg text-sm">
                                    <div>
                                        ...
                                        <span className="font-medium text-gray-900">
                                            {new Intl.NumberFormat(undefined, { style: 'currency', currency: formData.budget_currency || 'XOF', maximumFractionDigits: 0 }).format(quote.base_cost)}
                                        </span>
                                    </div>
                                    <div>
                                        <span className="text-gray-500 block mb-1">Assurance</span>
                                        <span className="font-medium text-gray-900">
                                            {new Intl.NumberFormat(undefined, { style: 'currency', currency: formData.budget_currency || 'XOF', maximumFractionDigits: 0 }).format(quote.insurance_cost)}
                                        </span>
                                    </div>
                                    <div className="border-t border-gray-100 pt-2 mt-2">
                                        <span className="text-gray-500 block mb-1">Sous-total HT</span>
                                        <span className="font-medium text-gray-900">
                                            {new Intl.NumberFormat(undefined, { style: 'currency', currency: formData.budget_currency || 'XOF', maximumFractionDigits: 0 }).format(quote.base_cost + quote.insurance_cost)}
                                        </span>
                                    </div>
                                    {quote.tax_cost > 0 && (
                                        <div>
                                            <span className="text-gray-500 block mb-1">TVA (18%)</span>
                                            <span className="font-medium text-gray-900">
                                                {new Intl.NumberFormat(undefined, { style: 'currency', currency: formData.budget_currency || 'XOF', maximumFractionDigits: 0 }).format(quote.tax_cost)}
                                            </span>
                                        </div>
                                    )}
                                    <div>
                                        ...
                                        <span className="font-medium text-gray-900 capitalize flex items-center gap-1">
                                            {formData.transport_mode === 'sea' ? <Ship className="w-3 h-3" /> : <Plane className="w-3 h-3" />}
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <span className="text-xs text-gray-500 uppercase tracking-wider font-medium">Itinéraire</span>
                                    <div className="flex items-center gap-2 mt-1 font-medium text-gray-900">
                                        {formData.origin_port} <ArrowLeft className="w-4 h-4 rotate-180 text-gray-400" /> {formData.destination_port}
                                    </div>
                                </div>
                                <div>
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">{t('rfq.form.cargoType')} *</label>
                                    <input
                                        type="text"
                                        name="cargo_type"
                                        required
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">{t('rfq.form.cargoDescription')}</label>
                                    <textarea
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Budget Cible</label>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-2">Date de départ souhaitée</label>
                                        <input
                                            type="date"
                                            name="preferred_departure_date"
                                            value={formData.preferred_departure_date ? new Date(formData.preferred_departure_date).toISOString().split('T')[0] : ''}
                                            onChange={handleChange}
                                            className="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary"
                                        />
                                    </div>
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white rounded-xl shadow-lg p-6 sticky top-6">
                            ...
                            <p className="text-sm text-gray-600 mb-6">
                                En confirmant, votre demande sera envoyée directement au transitaire sélectionné pour validation finale.
                            </p>
        ...
        <div className="max-w-7xl mx-auto space-y-8 pb-12">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">
                        {editMode ? 'Mettez à jour les informations de votre demande' : t('rfq.create.subtitle')}
                    </p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    <form ref={formRef} onSubmit={(e) => handleSubmit(e, false)} className="space-y-8">
                        ...
                        <div className="bg-blue-50 border-2 border-blue-200 rounded-lg p-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">{t('calculator.origin')} *</label>
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => {
                                                ...
                                                setFormData(prev => ({ ...prev, origin_port: e.target.value }));
                                            }}
                                            onFocus={() => {
                                                setOriginSearch('');
                                                setShowOriginDropdown(true);
                                            ...
                                            <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-auto">
                                                {filteredOrigins.map(loc => (
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            setFormData(prev => ({ ...prev, origin_port: loc.name }));
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">{t('calculator.destination')} *</label>
                                    <div className="relative">
                                        <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5 z-10" />
                                        <input
                                            type="text"
                                            ...
                                            onChange={(e) => {
                                                ...
                                                setFormData(prev => ({ ...prev, destination_port: e.target.value }));
                                            }}
                                            onFocus={() => {
                                                setDestSearch('');
                                                setShowDestDropdown(true);
                                            ...
                                            <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-auto">
                                                {filteredDestinations.map(loc => (
                                                    <button
                                                        ...
                                                        className="w-full text-left px-4 py-2 hover:bg-gray-50 text-sm text-gray-700"
                                                        onClick={() => {
                                                            setFormData(prev => ({ ...prev, destination_port: loc.name }));
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-3">{t('calculator.transportMode')}</label>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {(Object.keys(TRANSPORT_MODE_INFO) as Array<keyof typeof TRANSPORT_MODE_INFO>).map((mode) => {
                                    ...
                                    return (
                                        <label
                                            key={mode}
                                            className={`relative flex flex-col p-5 border-2 rounded-lg cursor-pointer transition-all hover:shadow-lg ${isSelected
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center justify-between mb-4">
                                                <div className="flex items-center gap-3">
                                                    <Icon size={28} className={isSelected ? 'text-primary' : 'text-gray-600'} />
                                            ...
                                            <div className="space-y-2.5 mb-4">
                                                <div className="flex items-start gap-2">
                                                    ...
                                                    <div className="text-sm">
                                                        <span className="font-semibold text-gray-900">{t('calculator.measure')}: </span>
                                                        <span className="text-gray-700">{info.measurement}</span>
                                                    </div>
                                                </div>
                                                <div className="flex items-start gap-2">
                                                    <Clock size={18} className="text-orange-600 mt-0.5 flex-shrink-0" />
                                                    <div className="text-sm">
                                                        <span className="font-semibold text-gray-900">{t('calculator.duration')}: </span>
                                                        <span className="text-gray-700">{info.duration}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex flex-wrap gap-2 mb-4">
                                                {info.advantages.map((advantage) => (
                                                    <span key={advantage} className="inline-flex items-center gap-1 bg-green-100 text-green-800 text-xs font-medium px-3 py-1 rounded-full">
                                                        <Check size={14} />
                                                        {advantage}
                                                    </span>
                                                ))}
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-3">{t('calculator.serviceType')}</label>
                                ...
                                {(Object.keys(SERVICE_TYPE_INFO) as Array<keyof typeof SERVICE_TYPE_INFO>).map((type) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`flex-1 py-2 text-sm font-medium rounded-md transition-all duration-200 ${isSelected
                            ...
                            <div className="bg-white border border-gray-200 rounded-lg p-4 transition-all duration-300">
                                <div className="flex items-center justify-between mb-3">
                                    <div className="flex items-center gap-2">
                                        <span className={`text-lg font-bold ${formData.service_type === 'express' ? 'text-orange-600' : 'text-green-600'}`}>
                                            {SERVICE_TYPE_INFO[formData.service_type].label}
                                        </span>
                                    </div>
                                    <div className={`text-xs px-2 py-1 rounded-full flex items-center gap-1 ${formData.service_type === 'express' ? 'bg-orange-100 text-orange-800' : 'bg-green-100 text-green-800'}`}>
                                        <Clock size={12} />
                                        {SERVICE_TYPE_INFO[formData.service_type].transitNote}
                                    </div>
                                </div>
                        ...
                        <div className="bg-gray-50 p-6 rounded-lg border border-gray-100">
                            ...
                            {formData.transport_mode === 'sea' ? (
                                <div className="space-y-4">
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-2">{t('calculator.measurementUnit')}</label>
                                        <select
                                            value={dimensionUnit}
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-2">{t('calculator.dimensions')}</label>
                                        <div className="grid grid-cols-3 gap-3">
                                            <div>
                                                <label className="block text-xs text-gray-600 mb-1">{t('calculator.length')}</label>
                                                <input
                                                    type="number"
                                                    step="0.01"
                                                    value={length}
                                            ...
                                            <div>
                                                <label className="block text-xs text-gray-600 mb-1">{t('calculator.width')}</label>
                                                <input
                                                    type="number"
                                                    step="0.01"
                                                    value={width}
                                            ...
                                            <div>
                                                <label className="block text-xs text-gray-600 mb-1">{t('calculator.height')}</label>
                                                <input
                                                    type="number"
                                                    step="0.01"
                                                    value={height}
                                        ...
                                        <div className="bg-green-50 border border-green-200 rounded-md p-3">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-sm font-medium text-green-900">
                                                    {t('calculator.volumeCalculated')}: <strong>{calculatedCBM.toFixed(4)} CBM</strong>
                                                </span>
                                            </div>
                                        </div>
                                    )}
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">{t('calculator.weightKG')}</label>
                                    <input
                                        type="number"
                                        name="weight_kg"
                                        step="0.1"
                        ...
                        <div className="bg-white border border-gray-200 rounded-lg p-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {[
                                    { id: 'insurance', label: 'Assurance (Garantie Plateforme)', desc: 'Protection complète contre la perte ou les dommages', icon: ShieldCheck, color: 'blue' },
                                    { id: 'priority', label: 'Traitement Prioritaire', desc: 'Accélérez le traitement de votre dossier', icon: Zap, color: 'orange' },
                                    { id: 'packaging', label: 'Emballage Renforcé', desc: 'Protection supplémentaire pour vos colis fragiles', icon: Box, color: 'indigo' },
                                    { id: 'inspection', label: 'Inspection Qualité', desc: 'Vérification de la conformité de la marchandise', icon: ClipboardCheck, color: 'green' },
                                    { id: 'customs_clearance', label: 'Dédouanement', desc: 'Gestion des formalités douanières', icon: FileCheck, color: 'teal' },
                                    { id: 'door_to_door', label: 'Door to Door', desc: 'Livraison jusqu\'à l\'adresse finale', icon: Truck, color: 'cyan' },
                                    { id: 'storage', label: 'Stockage', desc: 'Entreposage temporaire', icon: Warehouse, color: 'slate' }
                                ].map((service) => {
                                    const Icon = service.icon;
                                    const isSelected = formData.services_needed?.includes(service.id as any);
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className={`p-2 rounded-full mr-4 ${isSelected
                                            ...
                                            <div className="flex-1">
                                                <span className={`font-bold block ${isSelected ? 'text-gray-900' : 'text-gray-700'}`}>
                                                    {service.label}
                                                </span>
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        setFormData(prev => {
                                                            ...
                                                            if (isChecked) {
                                                                return { ...prev, services_needed: [...currentServices, service.id as any] };
                                                            } else {
                                                                return { ...prev, services_needed: currentServices.filter(id => id !== service.id) };
                                                            }
                                                        });
                                                    }}
                        ...
                        <div className="bg-white border border-gray-200 rounded-lg p-6">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">Exigences Spéciales</label>
                                <textarea
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                            ...
                            <div className="space-y-4 mb-6">
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="font-medium capitalize">{t(`calculator.modes.${formData.transport_mode}`)}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div className="flex-1 space-y-4">
                                            <div>
                                                ...
                                                <p className="font-medium text-gray-900">{formData.origin_port || '-'}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-gray-500 uppercase">Destination</p>
                                                <p className="font-medium text-gray-900">{formData.destination_port || '-'}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="space-y-3 pt-2">
                                ...
                                >
                                    {loading ? (
                                        ...
                                        <>
                                            ...
                                            {editMode ? 'Mettre à jour et Publier' : t('rfq.create.submit')}
                                        </>
                                    )}
                                </button>
                                ...
                                >
                                    ...
                                    {editMode ? 'Enregistrer les modifications' : 'Enregistrer Brouillon'}
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 text-center animate-in fade-in zoom-in duration-200">
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            >
                                {editMode ? 'Continuer les modifications' : 'Créer une autre demande'}
                            </button>
                        </div>
                    </div>
                </div>
