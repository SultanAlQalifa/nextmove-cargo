  ...
  const loadRFQs = async () => {
    try {
      ...
      if (filter === "offers_received") {
        // For "offers_received", we fetch all and filter by count > 0 locally
        // This ensures validation even if status wasn't updated in DB
        const allData = await rfqService.getMyRFQs({});
        data = allData.filter((r) => (r.offers_count || 0) > 0);
      } else {
        const filters = filter === "all" ? {} : { status: filter };
        data = await rfqService.getMyRFQs(filters);
      }
              ...
              <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                <div className="flex-1 w-full">
                  <div className="flex items-center justify-between md:justify-start gap-4 mb-4">
                    <div className="flex items-center gap-3">
                      <div
                        className={`p-2.5 rounded-xl ${rfq.transport_mode === "air" ? "bg-blue-50 text-blue-600" : "bg-indigo-50 text-indigo-600"}`}
                      >
                    ...
                    <div className="md:hidden">
                      ...
                      {(rfq.offers_count || 0) > 0 && (
                        <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-green-100 text-green-800 border border-green-200 ml-2 animate-pulse">
                          {rfq.offers_count} Offre{rfq.offers_count! > 1 ? 's' : ''}
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex flex-col items-end gap-3 w-full md:w-auto border-t md:border-t-0 border-gray-100 pt-4 md:pt-0">
                  <div className="hidden md:block">
                    {getStatusBadge(rfq.status)}
                    {(rfq.offers_count || 0) > 0 && (
                      <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-green-100 text-green-800 border border-green-200 ml-2 shadow-sm">
                        {rfq.offers_count} Offre{rfq.offers_count! > 1 ? 's' : ''}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-2 w-full md:w-auto">
                    <button
                      onClick={() =>
                        navigate(`/dashboard/client/rfq/${rfq.id}`)
                      }
                    ...
                    {rfq.status === "draft" && (
                      <>
                        <button
                          onClick={() =>
                            navigate(`/dashboard/client/rfq/${rfq.id}/edit`)
                          }
