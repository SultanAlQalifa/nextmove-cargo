        ...
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <div className="w-full sm:w-auto">
            ...
            >
              ...
              <option value="closing_soon">Bientôt Fermé</option>
          ...
          <div className="w-full sm:w-auto">
            <select
              ...
              onChange={(e) =>
                setFilters({
                  ...filters,
                  transport_mode: (e.target.value as any) || undefined,
                })
              }
            >
      ...
      {loading ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {consolidations.map((item) => (
            <div
              key={item.id}
              className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-slate-200 dark:border-gray-700 hover:shadow-md transition-shadow overflow-hidden group"
            >
              <div className="p-6">
                ...
                {item.initiator && (
                  <div className="flex items-center gap-2 mb-3">
                    <div className="text-sm font-semibold text-gray-700 dark:text-gray-300">
                      {item.initiator.company_name || "Transporteur"}
                    </div>
                    {item.initiator.rating && (
                      <div className="flex items-center text-xs text-yellow-500 gap-0.5">
                        <span className="font-bold">{item.initiator.rating}</span> ★
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="space-y-3 mb-6">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <ConsolidationProgressBar
                      current={item.current_load_cbm || 0}
                      total={item.total_capacity_cbm || 1}
                    />
                  </div>
                </div>
              </div>
              ...
              {item.services_requested &&
                item.services_requested.length > 0 && (
                    ...
                    <div className="flex flex-wrap gap-2">
                      {item.services_requested.map((serviceId) => {
                        ...
                        return (
                          <div
                            key={serviceId}
                            className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-slate-100 dark:bg-gray-700 text-xs text-slate-600 dark:text-slate-300"
...
function ConsolidationProgressBar({ current, total }: { current: number; total: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="w-24 h-2 bg-slate-100 dark:bg-gray-700 rounded-full overflow-hidden">
        <div
          ref={setBarEl}
          className="h-full bg-blue-500 rounded-full w-[var(--progress)] transition-all duration-500"
        />
      </div>
      <span className="font-medium text-slate-900 dark:text-white">
