...
import { Ship, Plane, ArrowRight, Calendar, MapPin, Box, Filter } from 'lucide-react';
...
export default function MarketplaceShowcase() {
    ...
    const [activeTab, setActiveTab] = useState<'all' | 'groupage' | 'expedition'>('all');
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {filteredOffers.map((offer) => {
                        ...
                        const isExpeditionView = activeTab === 'expedition' || (activeTab === 'all' && offer.transport_mode === 'air');
                        const isGroupageView = !isExpeditionView;

                        const priceUnit = offer.transport_mode === 'sea' ? '/ CBM' : '/ kg';
                        const priceValue = offer.transport_mode === 'sea' ? offer.price_per_cbm : offer.price_per_kg;
                        const formattedPrice = priceValue
                            ? `${priceValue.toLocaleString()} ${offer.currency} ${priceUnit}`
                            : 'Sur devis';

                        // Calculate duration
                        const duration = offer.departure_date && offer.arrival_date
                            ? Math.ceil((new Date(offer.departure_date).getTime() - new Date(offer.arrival_date).getTime()) / (1000 * 60 * 60 * 24))
                            : null;

                        return (
                            <div key={offer.id} className={`group relative rounded-[2.5rem] p-8 transition-all duration-500 hover:-translate-y-2 overflow-hidden flex flex-col h-full ${isGroupageView
                                ...
                                <div className="relative z-10 flex flex-col gap-6 mb-8">
                                    <div className="flex justify-between items-start">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg transform group-hover:rotate-6 transition-transform duration-500 ${isGroupageView
                                                ...
                                                {offer.transport_mode === 'sea' ? <Ship className="w-7 h-7" /> : <Plane className="w-7 h-7" />}
                                            </div>
                                            <div className="flex flex-col">
                                                <span className={`text-xs font-bold uppercase tracking-wider ${isGroupageView ? 'text-slate-400' : 'text-slate-500'}`}>
                                                    {offer.transport_mode === 'sea' ? t('calculator.sea.label') : t('calculator.air.label')}
                                                </span>
                                                <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full w-fit mt-1 ${isGroupageView ? 'bg-emerald-500/20 text-emerald-400' : 'bg-blue-100 text-blue-600'
                                                    }`}>
                                                    Standard
                                                </span>
                                            </div>
                                        ...
                                        <div className={`flex flex-col items-end`}>
                                            <span className={`text-xs font-bold uppercase tracking-wider mb-1 ${isGroupageView ? 'text-slate-400' : 'text-slate-400'
                                                }`}>
                                                {t('marketplace.officialRate')}
                                            </span>
                                            <div className={`px-4 py-2 rounded-xl text-lg font-bold border backdrop-blur-md ${isGroupageView
                                    ...
                                    {/* Route Title */}
                                    <div>
                                        <div className={`text-xs font-bold uppercase tracking-widest mb-2 ${isGroupageView
                                            ...
                                            {isGroupageView ? t('marketplace.offerGroupage') : t('marketplace.offerExpedition')}
                                        </div>
                                        <h3 className={`text-xl font-bold leading-tight ${isGroupageView ? 'text-white' : 'text-slate-900'
                                            }`}>
                                            {offer.origin_port} <span className="text-slate-400 mx-1">→</span> {offer.destination_port}
                                        </h3>
                                    </div>
                                </div>

                                {/* Detailed Timeline */}
                                <div className={`relative z-10 mb-8 p-5 rounded-3xl border ${isGroupageView
                                    ...
                                    <div className="flex items-center justify-between text-center relative">
                                        ...
                                        <div className="flex flex-col items-center z-10">
                                            <span className={`text-[10px] font-bold uppercase tracking-wider mb-1 ${isGroupageView ? 'text-slate-500' : 'text-slate-400'}`}>{t('marketplace.departure')}</span>
                                            <span className={`text-sm font-bold ${isGroupageView ? 'text-white' : 'text-slate-900'}`}>
                                        ...
                                        <div className="flex-1 px-4 flex flex-col items-center relative -mt-1">
                                            <span className={`text-[10px] font-medium mb-1 ${isGroupageView ? 'text-emerald-400' : 'text-blue-500'}`}>
                                                {duration ? `${duration} ${t('calculator.days')}` : 'Direct'}
                                            </span>
                                            <div className="w-full h-px bg-slate-300 dark:bg-slate-700 relative">
                                                <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-2 h-2 rounded-full ${isGroupageView ? 'bg-emerald-500' : 'bg-blue-500'
                                                    }`}></div>
                                            </div>
                                        </div>
                                        ...
                                        {/* Arrival */}
                                        <div className="flex flex-col items-center z-10">
                                            <span className={`text-[10px] font-bold uppercase tracking-wider mb-1 ${isGroupageView ? 'text-slate-500' : 'text-slate-400'}`}>{t('marketplace.arrival')}</span>
                                            <span className={`text-sm font-bold ${isGroupageView ? 'text-white' : 'text-slate-900'}`}>
                                            ...
                                            <div className="w-full">
                                                <div className="flex justify-between mb-2.5">
                                                    ...
                                                    <span className="text-sm font-bold text-white">
                                                        {offer.transport_mode === 'sea'
                                                            ? Math.max(0, (offer.total_capacity_cbm || 0) - (offer.current_load_cbm || 0))
                                                            : Math.max(0, (offer.total_capacity_kg || 0) - (offer.current_load_kg || 0))
                                                        }
                                                ...
                                                <div className="h-2 w-full rounded-full overflow-hidden bg-slate-800">
                                                    <div
                                                        ...
                                                        style={{
                                                            width: offer.transport_mode === 'sea'
                                ...
                                {/* Action Button */}
                                <div className={`mt-6 pt-6 border-t border-dashed ${isGroupageView ? 'border-slate-800' : 'border-slate-200'}`}>
                                    {isGroupageView ? (
                                        <Link
                                            to="/register"
