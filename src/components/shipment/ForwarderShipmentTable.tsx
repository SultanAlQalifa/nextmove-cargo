...
import { MoreHorizontal, MapPin, Calendar, Package, ArrowRight, Eye } from 'lucide-react';
...
export default function ForwarderShipmentTable({ shipments, onViewDetails, onUpdateStatus }: ForwarderShipmentTableProps) {
                            ...
                            <tr key={shipment.id} className="hover:bg-gray-50/50 transition-colors group">
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="flex flex-col">
                                        <span className="font-mono font-bold text-sm text-gray-900">{shipment.tracking_number}</span>
                                        <span className="text-xs text-gray-400 flex items-center gap-1 mt-1">
                                            <Package className="w-3 h-3" />
                                            {shipment.cargo.packages} colis • {shipment.cargo.weight} kg
                                        </span>
                                    </div>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="flex items-center gap-2 text-sm">
                                        <span className="font-medium text-gray-700">{shipment.origin.port}</span>
                                        <ArrowRight className="w-3 h-3 text-gray-400" />
                                        <span className="font-medium text-gray-700">{shipment.destination.port}</span>
                                    </div>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-sm text-gray-600">Client #123</span>
                                    </div>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <span className={`px-2.5 py-1 rounded-full text-xs font-bold border ${getStatusStyle(shipment.status)}`}>
