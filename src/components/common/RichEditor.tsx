...
export default function RichEditor({ value, onChange, placeholder, className = '' }: RichEditorProps) {
    ...
    useEffect(() => {
        ...
        if (editorRef.current && value !== editorRef.current.innerHTML && !isLocked.current) {
    ...
    const ToolbarButton = ({ icon: Icon, command, arg, title, active = false }: any) => (
        <button
            type="button"
        ...
        <div className={`flex flex-col border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden bg-white dark:bg-gray-800 ${className}`}>
            ...
            <div className="flex flex-wrap items-center gap-1 p-2 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50">
                ...
                <div className="flex items-center gap-0.5 border-r border-gray-200 dark:border-gray-700 pr-2 mr-1">
                    ...
                    <ToolbarButton icon={Type} command="formatBlock" arg="P" title="Paragraphe" />
                </div>
            ...
            {/* Editor Area */}
            <div className="flex-1 overflow-y-auto bg-white dark:bg-gray-800 cursor-text group" onClick={() => editorRef.current?.focus()}>
                <div
                    ref={editorRef}
                    contentEditable
                    onInput={handleInput}
