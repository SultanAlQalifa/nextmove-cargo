...
import { MessageCircle, X, Send, Minus, Maximize2, Search, ChevronLeft, Trash2 } from 'lucide-react';
...
export default function ChatWidget() {
    const { user } = useAuth();
    const { isOpen, setIsOpen, selectedConversation, setSelectedConversation, recipientDetails } = useChat();
    ...
    useEffect(() => {
        if (selectedConversation) {
            ...
            const subscription = messageService.subscribeToMessages(selectedConversation, (payload) => {
    ...
    const getOtherParticipant = (conv: Conversation) => {
        return conv.participants.find(p => p.user_id !== user?.id)?.user || { full_name: 'Utilisateur', email: '' };
    };

    if (!user) return null;

    return (
        <div className={`fixed bottom-6 right-6 z-50 flex flex-col items-end transition-all duration-300 ${isMinimized ? 'w-auto' : 'w-full max-w-sm'}`}>
            {/* Toggle Button */}
            {!isOpen && (
                <button
            ...
            {/* Chat Window */}
            {isOpen && (
                <div className={`bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col transition-all duration-300 ${isMinimized ? 'h-14 w-72' : 'h-[500px] w-full'}`}>
                    {/* Header */}
                    <div className="bg-primary text-white p-4 flex items-center justify-between cursor-pointer" onClick={() => !selectedConversation && setIsMinimized(!isMinimized)}>
                                ...
                                <>
                                    <div className="flex-1 overflow-y-auto p-4 bg-gray-50 space-y-4">
                                        {messages.map((msg) => (
                                            ...
                                            >
                                                <div className={`max-w-[80%] rounded-2xl p-3 text-sm ${msg.sender_id === user.id
                                                    ? 'bg-primary text-white rounded-br-none'
                                                    : 'bg-white text-gray-700 border border-gray-100 rounded-bl-none shadow-sm'
                                                    }`}>
                                                    <p>{msg.content}</p>
                                                    <p className={`text-[10px] mt-1 ${msg.sender_id === user.id ? 'text-white/70' : 'text-gray-400'}`}>
