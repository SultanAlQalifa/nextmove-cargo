...
export default function Login() {
    ...
    const handleResetPassword = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (error) throw error;
            setResetMessage({ type: 'success', text: 'Email de réinitialisation envoyé ! Vérifiez votre boîte mail.' });
        } catch (err: any) {
            setResetMessage({ type: 'error', text: err.message || 'Erreur lors de l\'envoi de l\'email.' });
        } finally {
    ...
    const handleQuickLogin = async (emailVal: string, passwordVal: string) => {
        ...
        try {
            ...
            if (error) {
                // 2. If invalid credentials, try to sign up (auto-create for demo)
                if (error.message.includes('Invalid login credentials') || error.message.includes('Email not confirmed')) {
                    // console.log('Login failed, attempting to create demo account...');

                    // Determine role based on email
                    let role = 'client';
                    if (emailVal.includes('forwarder')) role = 'forwarder';
                    if (emailVal.includes('driver')) role = 'driver';
                    ...
                    const { data: signUpData, error: signUpError } = await supabase.auth.signUp({
                        ...
                        options: {
                            data: {
                                full_name: role === 'driver' ? 'Test Driver' : (role === 'forwarder' ? 'Test Forwarder' : 'Test Client'),
                                role: role
                            }
                        }
                    });
                    ...
                    // If signup successful (or requires confirmation), try login again immediately
                    // Note: If email confirmation is required, this might still fail, but for dev it usually works if disabled
                    ...
                    if (retryError) {
                        // If still failing, it might be email confirmation. 
                        // For demo purposes, we can't easily fix that without admin access, 
                        // but usually Supabase dev projects have confirmation off.
                        throw retryError;
                    }
                } else {
                    throw error;
            ...
            setError(err.message === 'Invalid login credentials'
                ? 'Compte non trouvé ou mot de passe incorrect. Avez-vous créé le compte sur /test-accounts ?'
                : err.message);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="h-screen flex bg-white overflow-hidden">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl max-w-md w-full p-8 relative border border-slate-100 dark:border-slate-800">
                        ...
                        <div className="text-center mb-8">
                            ...
                            <h3 className="text-2xl font-bold text-slate-900 dark:text-white">Mot de passe oublié ?</h3>
                        ...
                        {resetMessage && (
                            <div className={`mb-6 p-4 rounded-2xl text-sm flex items-center gap-3 ${resetMessage.type === 'success'
            ...
            <div className="hidden lg:flex lg:w-1/2 relative bg-slate-900 overflow-hidden">
                ...
                <img
                    src={settings?.images.login_background || "https://images.unsplash.com/photo-1578575437130-527eed3abbec?q=80&w=2070&auto=format&fit=crop"}
                    alt="Logistics"
                ...
                <div className="relative z-10 flex flex-col justify-between p-16 text-white w-full h-full">
                    <div className="flex items-center space-x-4">
                        ...
                        <span className="text-2xl font-bold tracking-tight">{settings?.platform_name || 'NextMove Cargo'}</span>
                    </div>
                    ...
                    <div className="mb-8">
                        <blockquote className="space-y-8">
                            ...
                            <footer className="flex items-center gap-4">
                                ...
                                <div>
                                    <div className="text-lg font-bold text-white">Amadou Diallo</div>
                                    <div className="text-sm text-blue-300 font-medium">CEO Global Import SA</div>
                                </div>
                            </footer>
                        </blockquote>
                    </div>
