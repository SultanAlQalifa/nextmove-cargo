    ...
    useEffect(() => {
        ...
        supabase.auth.getSession().then(({ data: { session } }) => {
            ...
            if (!session) {
                ...
                setMessage({ type: 'error', text: 'Lien invalide ou expiré. Veuillez refaire une demande.' });
            }
        });
    }, []);
            ...
            <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-2xl shadow-lg">
                ...
                {message && (
                    <div className={`p-4 rounded-xl flex items-center gap-3 ${message.type === 'success' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
                        {message.type === 'success' ? <CheckCircle className="w-5 h-5" /> : <Loader2 className="w-5 h-5 animate-spin" />}
                ...
                {session ? (
                    ...
                    <div className="mt-8 text-center">
                        <p className="text-gray-500 mb-4">
                            Si vous n'avez pas demandé de réinitialisation, vous pouvez ignorer cette page.
                        </p>
                        <button
