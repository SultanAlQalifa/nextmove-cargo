...
export default function AvailableRFQs() {
    ...
    const loadRFQs = async () => {
        try {
            setLoading(true);
            // Combine search term with filters if needed, or handle client-side filtering for search
            const data = await rfqService.getAvailableRFQs(filters);
            setRfqs(data);
        } catch (error) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        // Simple client-side search for demo purposes, or update filters to trigger API call
        // For now, let's just trigger a reload which uses the filters state
        // Ideally, we'd pass searchTerm to the API or filter locally
        loadRFQs();
    };
        ...
        <div className="space-y-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">Trouvez de nouvelles opportunités et soumettez vos offres.</p>
                </div>
            </div>
            ...
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <div className="flex gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
                    <button
                        onClick={() => setFilters({ ...filters, transport_mode: undefined })}
            ...
            {loading ? (
                ...
                <div className="text-center py-16 bg-white rounded-2xl border border-gray-100 border-dashed">
                    ...
                    <p className="text-gray-500 mt-2">Essayez de modifier vos filtres ou revenez plus tard.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 gap-6">
                    {filteredRfqs.map((rfq) => (
                        <div key={rfq.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-all group">
                            <div className="flex flex-col lg:flex-row gap-6 justify-between items-start lg:items-center">
                                {/* Route & Icon */}
                                <div className="flex items-start gap-4 flex-1">
                                    <div className={`p-3 rounded-xl ${rfq.transport_mode === 'air' ? 'bg-blue-50 text-blue-600' : 'bg-indigo-50 text-indigo-600'}`}>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-4 mt-2 text-sm text-gray-500">
                                            <span className="flex items-center gap-1.5">
                                                ...
                                                Départ: {rfq.preferred_departure_date ? new Date(rfq.preferred_departure_date).toLocaleDateString() : 'Flexible'}
                                            </span>
                                            <span className="flex items-center gap-1.5">
                                                <Clock className="w-4 h-4" />
                                                Publié le: {new Date(rfq.created_at).toLocaleDateString()}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex-1 grid grid-cols-2 gap-4 border-l border-r border-gray-50 px-6">
                                    <div>
                                        <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Marchandise</p>
                                        <p className="font-semibold text-gray-900 mt-1">{rfq.cargo_type}</p>
                                        <p className="text-sm text-gray-500">{rfq.quantity} unité(s)</p>
                                    </div>
                                    <div>
                                        <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Budget Cible</p>
                                        {rfq.budget_amount ? (
                                            <p className="font-semibold text-green-700 mt-1">
                                                {rfq.budget_amount.toLocaleString()} {rfq.budget_currency}
                                            </p>
                                        ) : (
                                            <p className="text-sm text-gray-400 italic mt-1">Non spécifié</p>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div>
                                    <button
                                        onClick={() => navigate(`/dashboard/forwarder/rfq/${rfq.id}/offer`)}
