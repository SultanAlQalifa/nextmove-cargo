...
export default function AdminDashboard() {
    ...
    const [stats, setStats] = useState({
        users: { value: 0, trend: '+0%', trendUp: true, label: 'Utilisateurs totaux' },
        shipments: { value: 0, trend: '+0%', trendUp: true, label: 'Expéditions actives' },
        revenue: { value: 0, trend: '+0%', trendUp: true, label: 'Revenu total' },
        conversion: { value: 0, trend: '0%', trendUp: true, label: 'Taux de conversion' }
    });
    ...
    const loadDashboardData = async () => {
        ...
        try {
            ...
            const { data: revData, error: revError } = await supabase.rpc('get_revenue_chart', { time_range: timeRange });
            if (revData) {
                // Map to chart format
            ...
            const { data: shipData, error: shipError } = await supabase.rpc('get_shipment_stats');
            if (shipData) {
    ...
    const handleDownloadReport = () => {
        ...
        link.setAttribute("download", `rapport_activite_${new Date().toISOString().split('T')[0]}.csv`);
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <Link to="/dashboard/admin/personnel" className="block group">
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all group-hover:border-primary/20">
                                ...
                                <h3 className="text-gray-500 text-sm font-medium">Personnel & Rôles</h3>
                                <p className="text-3xl font-bold text-gray-900 mt-1">{stats.users.value > 0 ? Math.ceil(stats.users.value * 0.1) : 0}</p>
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <span className={`text-xs font-medium px-2 py-1 rounded-full flex items-center gap-1 ${stats.users.trendUp ? 'text-green-600 bg-green-50' : 'text-red-600 bg-red-50'}`}>
                                    {stats.users.trendUp ? <ArrowUpRight className="w-3 h-3" /> : <ArrowDownRight className="w-3 h-3" />} {stats.users.trend}
                                </span>
                            </div>
                            <h3 className="text-gray-500 text-sm font-medium">Utilisateurs Totaux</h3>
                            <p className="text-3xl font-bold text-gray-900 mt-1">{stats.users.value.toLocaleString()}</p>
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <span className={`text-xs font-medium px-2 py-1 rounded-full flex items-center gap-1 ${stats.revenue.trendUp ? 'text-green-600 bg-green-50' : 'text-red-600 bg-red-50'}`}>
                                    {stats.revenue.trendUp ? <ArrowUpRight className="w-3 h-3" /> : <ArrowDownRight className="w-3 h-3" />} {stats.revenue.trend}
                                </span>
                            </div>
                            <h3 className="text-gray-500 text-sm font-medium">Revenu Mensuel</h3>
                            <p className="text-3xl font-bold text-gray-900 mt-1">{formatCurrency(stats.revenue.value, currency)}</p>
                            <p className="text-xs text-gray-400 mt-2">{stats.revenue.label}</p>
                        </div>
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <span className={`text-xs font-medium px-2 py-1 rounded-full flex items-center gap-1 ${stats.conversion.trendUp ? 'text-green-600 bg-green-50' : 'text-red-600 bg-red-50'}`}>
                                    {stats.conversion.trendUp ? <ArrowUpRight className="w-3 h-3" /> : <ArrowDownRight className="w-3 h-3" />} {stats.conversion.trend}
                                </span>
                            </div>
                            <h3 className="text-gray-500 text-sm font-medium">Taux de Conversion</h3>
                            <p className="text-3xl font-bold text-gray-900 mt-1">{stats.conversion.value}%</p>
                            <p className="text-xs text-gray-400 mt-2">{stats.conversion.label}</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 min-w-0">
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500">Aperçu des 12 derniers mois</p>
                                </div>
                                <select className="bg-gray-50 border-none text-sm font-medium text-gray-600 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-primary/20">
                                        ...
                                        <ResponsiveContainer width="100%" height={320} minWidth={0}>
                                            <AreaChart data={revenueData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                                                ...
                                                <Tooltip
                                                    ...
                                                    formatter={(value: number) => [formatCurrency(value, currency), 'Revenu']}
                                                />
                                                <Area
                                                    type="monotone"
                                                    dataKey="value"
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 min-w-0">
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    <h3 className="text-lg font-bold text-gray-900 mb-1">État des Expéditions</h3>
                                    <p className="text-sm text-gray-500">Répartition par statut actuel</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-2xl font-bold text-gray-900">{shipmentStatusData.reduce((acc, curr) => acc + curr.value, 0)}</p>
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                            ...
                            <div className="space-y-3">
                                ...
                                >
                                    ...
                                    <span className="text-sm font-medium text-gray-700 group-hover:text-primary">Vérifier Transitaires</span>
                                </Link>
                                ...
                                >
                                    ...
                                    <span className="text-sm font-medium text-gray-700 group-hover:text-primary">Gérer RFQ</span>
                                </Link>
                                ...
                                >
                                    ...
                                    <span className="text-sm font-medium text-gray-700 group-hover:text-primary">Suivi Paiements</span>
                                </Link>
                                ...
                                >
                                    ...
                                    <span className="text-sm font-medium text-gray-700 group-hover:text-primary">Paramètres Système</span>
                                </Link>
                            </div>
                        </div>
                    </div>
