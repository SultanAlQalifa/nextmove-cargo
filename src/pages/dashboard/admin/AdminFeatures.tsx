...
export default function AdminFeatures() {
    ...
    const handleDelete = async (id: string) => {
        if (confirm('Êtes-vous sûr de vouloir supprimer cette fonctionnalité ? Elle ne sera plus disponible pour les nouveaux plans.')) {
            try {
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nom & Clé</th>
                                ...
                                <tr key={feature.id} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center">
                                            <div className={`p-2 rounded-lg mr-3 ${feature.category === 'core' ? 'bg-purple-50 text-purple-600' : feature.category === 'usage' ? 'bg-blue-50 text-blue-600' : 'bg-gray-100 text-gray-600'}`}>
                                                {feature.type === 'limit' ? <Hash className="w-4 h-4" /> : <ToggleLeft className="w-4 h-4" />}
                                            </div>
                                            <div>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="text-sm text-gray-500 max-w-xs truncate" title={feature.description}>
                                            {feature.description}
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${feature.type === 'limit' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
                                            }`}>
                                            {feature.type === 'limit' ? 'Limite Numérique' : 'Oui/Non'}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {feature.type === 'boolean' ? (
                                            feature.defaultValue ? 'Activé' : 'Désactivé'
                                        ) : (
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl animate-in fade-in zoom-in duration-200">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Clé unique (ID technique)</label>
                                <input
                                    type="text"
                                    required
                                    value={formData.id}
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select
                                        value={formData.type}
                                        onChange={(e) => setFormData({ ...formData, type: e.target.value as 'boolean' | 'limit', defaultValue: e.target.value === 'limit' ? 0 : false })}
                                    ...
                                    <div className="flex items-center gap-4 mt-2">
                                        <label className="flex items-center gap-2 cursor-pointer">
                                            <input
                                                ...
                                                onChange={() => setFormData({ ...formData, defaultValue: true })}
                                        ...
                                        <label className="flex items-center gap-2 cursor-pointer">
                                            <input
                                                ...
                                                onChange={() => setFormData({ ...formData, defaultValue: false })}
                                                className="text-primary focus:ring-primary"
                                            />
