...
import { Plus, Edit2, Trash2, DollarSign, Percent, Shield, Briefcase, AlertCircle, X, Save, Clock, AlertTriangle, Warehouse, FileText } from 'lucide-react';
...
export default function AdminFees() {
    ...
    const getCategoryIcon = (category: string) => {
        switch (category) {
            ...
            case 'guarantee': return AlertCircle;
    ...
    const getCategoryColor = (category: string) => {
        switch (category) {
            ...
            case 'guarantee': return 'bg-orange-50 text-orange-600';
            case 'management': return 'bg-purple-50 text-purple-600';
            case 'storage': return 'bg-amber-50 text-amber-600';
            case 'penalty': return 'bg-red-50 text-red-600';
            case 'tax': return 'bg-gray-100 text-gray-600';
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {fees.map((fee) => {
                        ...
                        return (
                            <div key={fee.id} className={`bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow relative overflow-hidden ${!fee.isActive ? 'opacity-75' : ''}`}>
                                {!fee.isActive && (
                                ...
                                <div className="flex justify-between items-start mb-4">
                                    <div className={`p-3 rounded-xl ${getCategoryColor(fee.category)}`}>
                                        <Icon className="w-6 h-6" />
                                    </div>
                                    <div className="flex gap-2">
                                        <button onClick={() => handleOpenModal(fee)} className="p-2 text-gray-400 hover:text-primary hover:bg-primary/5 rounded-lg transition-colors">
                                ...
                                <div className="mb-4">
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ${fee.target === 'client' ? 'bg-green-100 text-green-700' : 'bg-indigo-100 text-indigo-700'}`}>
                                            {fee.target === 'client' ? 'Client' : 'Transitaire'}
                                        </span>
                                    </div>
                                    <p className="text-sm text-gray-500 h-10 line-clamp-2">{fee.description || "Aucune description"}</p>
                                </div>

                                <div className="flex flex-col mb-2">
                                    <div className="flex items-baseline gap-1">
                                        <span className="text-2xl font-bold text-gray-900">
                                            {fee.type === 'percentage' ? `${fee.value}%` : `${fee.value.toLocaleString()} XOF`}
                                        </span>
                                ...
                                <div className="flex flex-wrap gap-2 mt-3">
                                    {(fee.minAmount || fee.maxAmount) && (
                                        <>
                                            {fee.minAmount && <span className="text-[10px] text-gray-400 bg-gray-50 px-2 py-0.5 rounded-full">Min: {fee.minAmount.toLocaleString()}</span>}
                                            {fee.maxAmount && <span className="text-[10px] text-gray-400 bg-gray-50 px-2 py-0.5 rounded-full">Max: {fee.maxAmount.toLocaleString()}</span>}
                                        </>
                                    )}
                                </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl animate-in fade-in zoom-in duration-200 max-h-[90vh] overflow-y-auto custom-scrollbar">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div className="flex bg-gray-100 p-1 rounded-xl">
                                        <button
                                            type="button"
                                            onClick={() => setFormData({ ...formData, target: 'client' })}
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => setFormData({ ...formData, target: 'forwarder' })}
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Type de Valeur</label>
                                    <div className="flex bg-gray-100 p-1 rounded-xl">
                                        <button
                                            type="button"
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <div className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400">
                                            {formData.type === 'percentage' ? <Percent className="w-4 h-4" /> : <span className="text-xs font-bold">XOF</span>}
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-medium text-gray-500 mb-1">Min (Optionnel)</label>
                                    <input
                                        type="number"
                                ...
                                <div>
                                    <label className="block text-xs font-medium text-gray-500 mb-1">Max (Optionnel)</label>
                                    <input
                                        type="number"
                            ...
                            <div className="bg-gray-50 p-4 rounded-xl space-y-4">
                                <div className="flex items-center justify-between">
                                    <label className="text-sm font-medium text-gray-700">Frais Récurrent</label>
                                    <div className="relative inline-block w-10 mr-2 align-middle select-none transition duration-200 ease-in">
                                        ...
                                        />
                                        <label htmlFor="isRecurring" className={`toggle-label block overflow-hidden h-5 rounded-full cursor-pointer ${formData.isRecurring ? 'bg-primary' : 'bg-gray-300'}`}></label>
                                    </div>
                                </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 animate-in fade-in slide-in-from-top-2">
                                        <div>
                                            <label className="block text-xs font-medium text-gray-500 mb-1">
                                                {formData.category === 'penalty' ? 'Délai avant (heures)' : 'Période de grâce (heures)'}
                                            </label>
                                            <input
                                                type="number"
                                                ...
                                                className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-primary outline-none"
                                                placeholder={formData.category === 'penalty' ? "Ex: 24" : "Ex: 72"}
                                            />
                                        </div>
