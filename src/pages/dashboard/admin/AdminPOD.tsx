...
import { FileCheck, CheckCircle, XCircle, Eye, Download, Search, Calendar, X, Filter } from 'lucide-react';
...
export default function AdminPOD() {
    ...
    const handleExport = () => {
        const headers = [
            ...
            { label: 'Date Soumission', key: (item: POD) => new Date(item.submitted_at).toLocaleDateString() },
    ...
    const handleValidateConfirm = async (id: string, note: string) => {
        try {
            // console.log('Validating POD with note:', note);
    ...
    const handleRejectConfirm = async (id: string, reason: string) => {
        try {
            // console.log('Rejecting POD with reason:', reason);
            await podService.verifyPOD(id, 'rejected', reason);
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'verified':
                return <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-700">Vérifié</span>;
            case 'rejected':
                return <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-700">Rejeté</span>;
            default:
                return <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-700">En attente</span>;
        }
    };
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead className="bg-gray-50">
                            <tr>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Expédition</th>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Transitaire</th>
                            ...
                            {filteredPODs.map((pod) => (
                                <tr key={pod.id} className="hover:bg-gray-50">
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="text-sm font-medium text-gray-900">{pod.shipment_id}</div>
                                        <div className="text-xs text-gray-500">{pod.tracking_number}</div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs text-gray-500">({pod.documents.length})</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                ...
                {filteredPODs.length === 0 && (
                    <div className="text-center py-12">
                        ...
                        <h3 className="text-lg font-medium text-gray-900">Aucun POD trouvé</h3>
