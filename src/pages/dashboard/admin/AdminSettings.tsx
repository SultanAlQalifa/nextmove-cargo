...
import { Settings, Globe, Bell, Lock, Save, AlertTriangle, RefreshCw, User, Camera, Mail, Phone, Key } from 'lucide-react';
import { settingsService, SystemSettings } from '../../../services/settingsService';
import { useSettings } from '../../../contexts/SettingsContext';
import { useAuth } from '../../../contexts/AuthContext';
import { profileService, UserProfile } from '../../../services/profileService';

type SettingsTab = 'regionalization' | 'notifications' | 'security' | 'maintenance' | 'profile' | 'email';

export default function AdminSettings() {
    const { settings: globalSettings, loading: contextLoading, refreshSettings } = useSettings();
    ...
    const handleSave = async () => {
        ...
        try {
            if (activeTab === 'profile') {
                ...
                await settingsService.updateSettings('security', localSettings.security);
                ...
                if (passwordData.new) {
                    ...
                    if (passwordData.new.length < 12) {
                        throw new Error("Le mot de passe doit contenir au moins 12 caractères pour une sécurité maximale.");
                    }
                ...
                if (!localSettings) return;
                await settingsService.updateSettings(activeTab as any, localSettings[activeTab as keyof SystemSettings]);
                await refreshSettings();
    ...
    if (!globalSettings || !localSettings) {
        return (
            <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-gray-500 mb-6">Impossible de charger les paramètres du système.</p>
                <button
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                        {activeTab === 'profile' && userProfile && (
                            <div className="space-y-8">
                                ...
                                <div className="flex items-center gap-6 pb-6 border-b border-gray-100">
                                    ...
                                    <div>
                                        <h3 className="text-lg font-bold text-gray-900">{userProfile.full_name || 'Utilisateur'}</h3>
                                        <p className="text-gray-500 text-sm capitalize">{userProfile.role}</p>
                                    </div>
                                </div>
                        ...
                        {activeTab === 'regionalization' && (
                            <div className="space-y-6">
                                <h3 className="text-lg font-bold text-gray-900 mb-4">Paramètres de Régionalisation</h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => updateSetting('regionalization', 'default_currency', e.target.value)}
                                        ...
                                        >
                                            {localSettings.regionalization.supported_currencies.map(c => (
                                                <option key={c} value={c}>{c}</option>
                                            ))}
                                        </select>
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium text-gray-700">Langue par défaut</label>
                                        <select
                                            value={localSettings.regionalization.default_language}
                                            onChange={(e) => updateSetting('regionalization', 'default_language', e.target.value)}
                                            className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                                        >
                                            {localSettings.regionalization.supported_languages.map(l => (
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => updateSetting('regionalization', 'timezone', e.target.value)}
                                            className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                                        >
                                            {localSettings.regionalization.supported_timezones?.map(tz => (
                        ...
                        {activeTab === 'notifications' && (
                            <div className="space-y-6">
                                <h3 className="text-lg font-bold text-gray-900 mb-4">Configuration des Notifications</h3>
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                                        <div>
                                            <p className="font-medium text-gray-900">Notifications Email</p>
                                            <p className="text-sm text-gray-500">Envoyer des emails transactionnels</p>
                                        </div>
                                    ...
                                    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-500">Envoyer des alertes par SMS</p>
                                        </div>
                                        <label className="relative inline-flex items-center cursor-pointer">
                                            <input
                                                type="checkbox"
                        ...
                        {activeTab === 'security' && (
                            <div className="space-y-6">
                                <h3 className="text-lg font-bold text-gray-900 mb-4">Sécurité du Système</h3>
                                <div className="space-y-4">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="space-y-2">
                                            <label className="text-sm font-medium text-gray-700">Longueur min. mot de passe</label>
                                            <input
                                                type="number"
                                                value={localSettings.security.min_password_length}
                                                onChange={(e) => updateSetting('security', 'min_password_length', Number(e.target.value))}
                                                className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                                            />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-sm font-medium text-gray-700">Timeout Session (minutes)</label>
                                            <input
                                                type="number"
                                    ...
                                    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-500">Forcer le 2FA pour les admins</p>
                                        </div>
                                        <label className="relative inline-flex items-center cursor-pointer">
                                            <input
                                                type="checkbox"
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div className="space-y-2">
                                                <label className="block text-sm font-medium text-gray-700">
                                                    ...
                                                    <span className="ml-2 text-xs font-normal text-gray-500">(Laisser vide pour conserver l'actuel)</span>
                                                </label>
                                                <input
                                                    type="password"
                        ...
                        {activeTab === 'email' && (
                            <div className="space-y-6">
                                ...
                                <div className="p-4 bg-blue-50 text-blue-700 rounded-xl mb-6 text-sm">
                                    ...
                                    <p>Les identifiants SMTP sont stockés de manière sécurisée. Le mot de passe est masqué par défaut.</p>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium text-gray-700">Serveur SMTP (Host)</label>
                                        <input
                                            type="text"
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => updateSetting('email', 'smtp_port', Number(e.target.value))}
                                            className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                                            placeholder="587"
                                        />
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium text-gray-700">Mot de passe SMTP</label>
                                        <input
                                            type="password"
                                            value={localSettings.email.smtp_pass}
                                            onChange={(e) => updateSetting('email', 'smtp_pass', e.target.value)}
                                            className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                                            placeholder="••••••••"
                                        />
                                    </div>
                                ...
                                <div className="pt-6 border-t border-gray-100">
                                    <h4 className="text-sm font-bold text-gray-900 mb-4">Expéditeur par défaut</h4>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="space-y-2">
                        ...
                        {activeTab === 'maintenance' && (
                            <div className="space-y-6">
                                <h3 className="text-lg font-bold text-gray-900 mb-4">Maintenance & Debug</h3>
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between p-4 bg-red-50 rounded-xl border border-red-100">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm text-red-700">Rend le site inaccessible aux utilisateurs</p>
                                            </div>
                                        </div>
                                        <label className="relative inline-flex items-center cursor-pointer">
                                            <input
                                    ...
                                    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-500">Affiche plus de détails sur les erreurs</p>
                                        </div>
                                        <label className="relative inline-flex items-center cursor-pointer">
                                            <input
                                                type="checkbox"
                                                checked={localSettings.maintenance.debug_mode}
                                                onChange={(e) => updateSetting('maintenance', 'debug_mode', e.target.checked)}
                                                className="sr-only peer"
                                            />
                                            <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-primary/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"></div>
                                        </label>
