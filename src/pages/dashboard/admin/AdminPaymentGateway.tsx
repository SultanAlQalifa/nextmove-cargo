...
import { CreditCard, Settings, Power, Save, AlertCircle, Eye, EyeOff } from 'lucide-react';
...
export default function AdminPaymentGateway() {
    ...
    const handleSaveConfig = async () => {
        ...
        setSaving(true);
        try {
            const savedGateway = await paymentGatewayService.updateGateway(selectedGateway.id, selectedGateway);

            // Update local state with the saved gateway (important for getting the real ID if it was a new creation)
            setGateways(prev => prev.map(g => g.id === selectedGateway.id ? savedGateway : g));
            setSelectedGateway(savedGateway);

            success('Configuration enregistrée avec succès !');
        } catch (error: any) {
            console.error('Error saving gateway:', error);
            toastError(`Erreur lors de l'enregistrement: ${error.message || 'Une erreur est survenue'}`);
        } finally {
            setSaving(false);
        }
    };
                        ...
                        >
                            <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-3">
                                    <div className={`p-2 rounded-lg ${selectedGateway?.id === gateway.id ? 'bg-primary/10 text-primary' : 'bg-gray-50 text-gray-500'} `}>
                                        <CreditCard className="w-5 h-5" />
                                    </div>
                                    <h3 className="font-bold text-gray-900">{gateway.name}</h3>
                                </div>
                                <div className={`w-3 h-3 rounded-full ${gateway.is_active ? 'bg-green-500' : 'bg-gray-300'} `}></div>
                            </div>
                        ...
                        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                            <div className="flex items-center justify-between mb-6">
                                <div className="flex items-center gap-3">
                                    ...
                                    <h3 className="text-lg font-bold text-gray-900">Configuration {selectedGateway.name}</h3>
                                </div>
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation();
...
`}
                            ...
                            <div className="space-y-6">
                                <div className="flex items-center gap-2 p-4 bg-blue-50 text-blue-700 rounded-xl text-sm">
                                    ...
                                    <p>Assurez-vous d'utiliser les clés de test pour le développement et les clés de production pour le site en ligne.</p>
                                </div>
                                    ...
                                    <div className="flex items-center gap-4">
                                        <label className="relative inline-flex items-center cursor-pointer">
                                            <input
                                                ...
                                                onChange={(e) => setSelectedGateway({ ...selectedGateway, is_test_mode: e.target.checked })}
                                                className="sr-only peer"
                                            />
                                            <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-primary/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"></div>
                                            <span className="ml-3 text-sm font-medium text-gray-700">Mode Test (Sandbox)</span>
                                        </label>
                                    </div>
