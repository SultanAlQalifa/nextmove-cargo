...
import { FileText, Upload, Download, Trash2, Search, Filter, File, Loader2, Eye } from 'lucide-react';
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="relative">
                    ...
                    >
                        ...
                        <span>{uploading ? t('documents.uploading') : t('documents.upload')}</span>
                    </label>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
                {loading ? (
                    ...
                    <div className="p-12 text-center">
                        ...
                        <h3 className="text-lg font-medium text-slate-900 dark:text-white mb-1">{t('documents.empty.title')}</h3>
                        <p className="text-slate-500 dark:text-slate-400">{t('documents.empty.subtitle')}</p>
                    </div>
                ) : (
                    <div className="divide-y divide-slate-100 dark:divide-slate-700">
                        {filteredDocuments.map((doc) => (
                            <div key={doc.id} className="p-4 hover:bg-slate-50 dark:hover:bg-gray-700/50 transition-colors flex items-center justify-between group">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-3 text-xs text-slate-500 dark:text-slate-400 mt-1">
                                            ...
                                            <span>•</span>
                                            <span>{format(new Date(doc.created_at), 'dd MMM yyyy', { locale: getDateLocale() })}</span>
                                        </div>
                                    </div>
                                </div>
