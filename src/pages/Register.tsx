  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const options = {
        data: {
          role: role,
          transport_modes: role === "forwarder" ? transportModes : [],
          referral_code_used: referralCode, // Pass used code to backend
        },
      };
      ...
      if (data.user) {
        ...
        // If session is null, email/phone confirmation is required
        if (!data.session) {
          setSuccess(true);
        } else {
          navigate("/dashboard");
          ...
          <div className="max-w-md w-full text-center space-y-8">
            ...
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-500 mt-2">
                Veuillez cliquer sur le lien pour activer votre compte et
          ...
          <div className="mx-auto w-full max-w-md">
            ...
            <div className="mt-8">
              <form onSubmit={handleRegister} className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {roles.map((r) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`flex-shrink-0 p-3 rounded-xl transition-colors ${isSelected ? "bg-blue-500 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-500 group-hover:text-blue-500"}`}
                          >
                            <Icon className="h-5 w-5" />
                          </div>
                          <div className="ml-3">
                            <span
                              className={`block text-sm font-bold ${isSelected ? "text-slate-900 dark:text-white" : "text-slate-600 dark:text-slate-400"}`}
                            >
                              {r.label}
                            </span>
                          </div>
                ...
                {/* Transport Modes Selection (Only for Forwarder) */}
                {role === "forwarder" && (
                  <div className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div
                        onClick={() => {
                          const newModes = transportModes.includes("sea")
                            ? transportModes.filter((m) => m !== "sea")
                            : [...transportModes, "sea"];
                          setTransportModes(newModes);
                        }}
                        className={`cursor-pointer p-4 rounded-xl border-2 transition-all ${transportModes.includes("sea")
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`p-2 rounded-lg ${transportModes.includes("sea") ? "bg-blue-500 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-500"}`}
                          >
                            <Anchor className="w-5 h-5" />
                          </div>
                          <span
                            className={`font-bold ${transportModes.includes("sea") ? "text-blue-700 dark:text-blue-300" : "text-slate-600 dark:text-slate-400"}`}
                          >
                            Maritime
                          </span>
                        </div>
                      ...
                      <div
                        onClick={() => {
                          const newModes = transportModes.includes("air")
                            ? transportModes.filter((m) => m !== "air")
                            : [...transportModes, "air"];
                          setTransportModes(newModes);
                        }}
                        className={`cursor-pointer p-4 rounded-xl border-2 transition-all ${transportModes.includes("air")
                          ? "border-orange-500 bg-orange-50 dark:bg-orange-900/20 shadow-lg shadow-orange-500/10"
                          : "border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 hover:border-orange-200"
                          }`}
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`p-2 rounded-lg ${transportModes.includes("air") ? "bg-orange-500 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-500"}`}
                          >
                            <Send className="w-5 h-5 rotate-[-45deg]" />
                          </div>
                          <span
                            className={`font-bold ${transportModes.includes("air") ? "text-orange-700 dark:text-orange-300" : "text-slate-600 dark:text-slate-400"}`}
                          >
                            Aérien
                          </span>
                        </div>
                  ...
                  {authMethod === 'email' ? (
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-slate-500 ml-1">
                        Un SMS de vérification sera envoyé si requis.
                      </p>
                    </div>
                  )}
