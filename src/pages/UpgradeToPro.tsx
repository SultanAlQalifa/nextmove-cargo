          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
            {filteredPlans.map((plan) => {
              ...
              return (
                <div
                  key={plan.id}
                  className={`bg-white dark:bg-gray-900 rounded-[2.5rem] shadow-xl shadow-slate-200/50 dark:shadow-none border ${isPro ? "border-blue-500 ring-4 ring-blue-500/10" : "border-slate-100 dark:border-slate-800"} overflow-hidden flex flex-col hover:-translate-y-2 transition-all duration-300 relative`}
                >
                  {isPro && (
                            ...
                            <div className="space-y-3">
                              {catDefinitions.map((def) => {
                                ...
                                return (
                                  <div
                                    key={def.id}
                                    className={`flex items-start gap-3 ${!isIncluded ? "opacity-40 grayscale" : ""}`}
                                  >
                                    <div
                                      className={`mt-1 ${isIncluded ? "text-blue-600" : "text-slate-300"}`}
                                    >
                                      {isIncluded ? (
