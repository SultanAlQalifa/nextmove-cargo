...
export const messageService = {
     ...
     */
    sendMessage: async (conversationId: string, content: string): Promise<Message | null> => {
        const { data: { user } } = await supabase.auth.getUser();
        if (!user) return null;
     ...
     */
    startConversation: async (otherUserId: string, initialMessage: string): Promise<Conversation | null> => {
        const { data: { user } } = await supabase.auth.getUser();
        if (!user) return null;
