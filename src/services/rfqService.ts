...
export async function getMyRFQs(filters?: RFQFilters): Promise<RFQRequest[]> {
  ...
  // Apply filters... (omitted lines as they are unchanged)
  ...
  // Manual fetch of offer counts to avoid join issues with inconsistent schema
  // Manual fetch of offer counts to avoid join issues with inconsistent schema
  ...
  return rfqs.map((rfq: any) => {
    const count = offers?.filter((o) => o.rfq_id === rfq.id).length || 0;
    return {
      ...rfq,
      offers_count: count,
    };
...
export async function getRFQWithOffers(id: string): Promise<RFQWithOffers> {
  ...
  return {
    ...rfq,
    offers: (offers as unknown as OfferWithForwarder[]) || [],
 ...
 */
export async function getOffersForRFQ(
  rfqId: string,
): Promise<OfferWithForwarder[]> {
