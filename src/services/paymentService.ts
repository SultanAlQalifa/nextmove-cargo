...
export const paymentService = {
    ...
    initializePayment: async (shipmentId: string, amount: number, currency: string) => {
    ...
    confirmPayment: async (shipmentId: string, details: any, couponId?: string) => {
        ...
        try {
            ...
            if (couponId && user) {
                ...
                if (couponError) console.error('Error recording coupon usage:', couponError);
            }
            ...
            await fetchWithRetry(() =>
                supabase
                    .from('shipments')
                    .update({ status: isOffline ? 'pending_payment' : 'paid' })
                    .eq('id', shipmentId)
            );
        } catch (error) {
