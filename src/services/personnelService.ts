...
const PERMISSIONS: Permission[] = [
    // 📦 Shipments
    { id: 'shipments.view', label: 'Voir les expéditions', category: 'operations' },
    { id: 'shipments.create', label: 'Créer des expéditions', category: 'operations' },
    { id: 'shipments.edit', label: 'Modifier les expéditions', category: 'operations' },
    { id: 'shipments.delete', label: 'Supprimer des expéditions', category: 'operations' },
    { id: 'shipments.status', label: 'Changer le statut', category: 'operations' },
    ...
    { id: 'personnel.delete', label: 'Supprimer du personnel', category: 'users' },
    { id: 'personnel.roles', label: 'Gérer les rôles', category: 'users' },

    // 💰 Finance
    { id: 'finance.view', label: 'Voir le tableau de bord financier', category: 'finance' },
    { id: 'finance.create', label: 'Créer devis/factures', category: 'finance' },
    { id: 'finance.payments', label: 'Enregistrer des paiements', category: 'finance' },
    { id: 'finance.reports', label: 'Exporter des rapports', category: 'finance' },

    // 🎫 Support
    { id: 'support.view', label: 'Voir les tickets', category: 'support' },
    { id: 'support.respond', label: 'Répondre aux tickets', category: 'support' },
    { id: 'support.manage', label: 'Gérer les tickets (Fermer/Supprimer)', category: 'support' },

    // ⚙️ Settings
    { id: 'settings.view', label: 'Voir la configuration', category: 'settings' },
    { id: 'settings.manage', label: 'Modifier les paramètres système', category: 'settings' }
];
...
export const ROLE_FAMILIES: Record<RoleFamily, { label: string; permissions: string[] }> = {
    ...
    forwarder: {
        ...
        permissions: [
            ...
            'finance.view', 'finance.create', 'finance.reports',
...
export const personnelService = {
    ...
    getForwarders: async (): Promise<any[]> => {
        const { data, error } = await supabase
            .from('profiles')
            .select('*')
            .eq('role', 'forwarder')
    ...
    updateForwarderStatus: async (id: string, updates: { kyc_status?: string; subscription_status?: string; subscription_plan?: string }): Promise<void> => {
    ...
    addForwarderStaff: async (staff: Omit<StaffMember, 'id' | 'last_active'>): Promise<StaffMember> => {
        const { data: { user } } = await supabase.auth.getUser();
        if (!user) throw new Error('Not authenticated');
    ...
    addStaff: async (staff: Omit<StaffMember, 'id' | 'last_active'>): Promise<StaffMember> => {
    ...
    addStaffMember: async (data: Omit<StaffMember, 'id' | 'last_active'>): Promise<StaffMember> => {
...
function mapDbProfileToStaff(dbRecord: any): StaffMember {
    const staffRole = Array.isArray(dbRecord.staff_role) ? dbRecord.staff_role[0] : dbRecord.staff_role;
    return {
        id: dbRecord.id,
