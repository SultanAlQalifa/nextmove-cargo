...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_type
    WHERE typname = 'fund_call_status'
) THEN CREATE TYPE fund_call_status AS ENUM ('pending', 'approved', 'rejected', 'paid');
END IF;
IF NOT EXISTS (
    SELECT 1
    FROM pg_type
    WHERE typname = 'pod_status'
) THEN CREATE TYPE pod_status AS ENUM ('pending', 'verified', 'rejected');
END IF;
IF NOT EXISTS (
    SELECT 1
    FROM pg_type
    WHERE typname = 'fee_type'
) THEN CREATE TYPE fee_type AS ENUM ('percentage', 'fixed');
END IF;
IF NOT EXISTS (
    SELECT 1
    FROM pg_type
    WHERE typname = 'fee_target'
) THEN CREATE TYPE fee_target AS ENUM ('client', 'forwarder');
END IF;
IF NOT EXISTS (
    SELECT 1
    FROM pg_type
    WHERE typname = 'fee_category'
) THEN CREATE TYPE fee_category AS ENUM (
    'insurance',
    'guarantee',
    'management',
    'storage',
...
IF NOT EXISTS (
    SELECT 1
    FROM pg_type
    WHERE typname = 'gateway_provider'
) THEN CREATE TYPE gateway_provider AS ENUM ('wave', 'payoneer', 'stripe', 'orange_money');
END IF;
IF NOT EXISTS (
    SELECT 1
    FROM pg_type
    WHERE typname = 'billing_cycle'
) THEN CREATE TYPE billing_cycle AS ENUM ('monthly', 'yearly');
END IF;
END $$;
-- ═══ 1. FUND CALLS ═══
...
DROP POLICY IF EXISTS "Users can view own fund calls" ON fund_calls;
...
DROP POLICY IF EXISTS "Admins can view all fund calls" ON fund_calls;
...
DROP POLICY IF EXISTS "Forwarders/Clients can view related PODs" ON pods;
...
DROP POLICY IF EXISTS "Forwarders can create PODs" ON pods;
...
DROP POLICY IF EXISTS "Public read access to fees" ON fee_configs;
CREATE POLICY "Public read access to fees" ON fee_configs FOR
SELECT USING (true);
DROP POLICY IF EXISTS "Admins can manage fees" ON fee_configs;
...
DROP POLICY IF EXISTS "Public read access to settings" ON system_settings;
...
DROP POLICY IF EXISTS "Admins can manage roles" ON staff_roles;
...
-- Ensure UNIQUE constraint on name for staff_roles
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'staff_roles_name_key'
) THEN -- Check for duplicates before adding constraint
IF (
    SELECT COUNT(*)
    FROM (
            SELECT name
...
END IF;
END $$;
-- Seed initial roles with role_family awareness
DO $$ BEGIN IF EXISTS (
    SELECT 1
...
FROM (
        VALUES (
            ...
            (
                ...
                'Support',
                'Gestion des tickets',
                ARRAY ['support_view', 'support_manage', 'users_view']::text [],
                true
            ),
            (
                'finance',
                'Finance',
                'Gestion financière',
                ARRAY ['finance_view', 'finance_manage', 'users_view']::text [],
                true
            )
    ) AS v(id, name, description, permissions, is_system)
WHERE NOT EXISTS (
        SELECT 1
        FROM staff_roles sr
        WHERE sr.id = v.id
...
) THEN CREATE TYPE ticket_status AS ENUM ('open', 'in_progress', 'resolved', 'closed');
END IF;
IF NOT EXISTS (
    SELECT 1
    FROM pg_type
    WHERE typname = 'ticket_priority'
) THEN CREATE TYPE ticket_priority AS ENUM ('low', 'medium', 'high', 'urgent');
END IF;
END $$;
...
-- ═══ TRIGGERS ═══
DROP TRIGGER IF EXISTS update_fund_calls_updated_at ON fund_calls;
...
DROP TRIGGER IF EXISTS update_fee_configs_updated_at ON fee_configs;
