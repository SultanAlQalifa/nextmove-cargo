...
DROP POLICY IF EXISTS "Public can view rates" ON rates;
CREATE POLICY "Public can view rates" ON rates FOR
SELECT USING (true);
-- Only admins and owning forwarders can manage rates
DROP POLICY IF EXISTS "Forwarders can manage own rates" ON rates;
CREATE POLICY "Forwarders can manage own rates" ON rates FOR ALL USING (
    (
        select auth.uid()
    ) = forwarder_id
) WITH CHECK (
