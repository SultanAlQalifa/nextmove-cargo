...
UPDATE TO authenticated USING ((select auth.uid()) = client_id);
-- 2. FUND CALLS
-- ───────────────────────────────────────────────────────────────
...
SELECT TO authenticated USING ((select auth.uid()) = requester_id);
...
CREATE POLICY "Drivers can manage own PODs" ON public.shipment_pods FOR ALL TO authenticated USING ((select auth.uid()) = driver_id);
