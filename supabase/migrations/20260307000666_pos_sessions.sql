...
-- Policies
DROP POLICY IF EXISTS "Forwarders can manage their own stations" ON pos_stations;
CREATE POLICY "Forwarders can manage their own stations" ON pos_stations FOR ALL USING (
    forwarder_id = (
        select auth.uid()
    )
);
DROP POLICY IF EXISTS "Agents can manage their own sessions" ON pos_sessions;
CREATE POLICY "Agents can manage their own sessions" ON pos_sessions FOR ALL USING (
    agent_id = (
        select auth.uid()
    )
);