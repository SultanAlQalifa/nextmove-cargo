...
SELECT USING ((select auth.uid()) = driver_id);
...
UPDATE USING ((select auth.uid()) = driver_id);
...
CREATE POLICY "Drivers can manage own PODs" ON public.shipment_pods FOR ALL USING ((select auth.uid()) = driver_id);
