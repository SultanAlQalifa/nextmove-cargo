...
DROP POLICY IF EXISTS "Drivers can update status of assigned shipments" ON public.shipments;
...
DROP POLICY IF EXISTS "Drivers can view events for assigned shipments" ON public.shipment_events;
...
DROP POLICY IF EXISTS "Drivers can create events for assigned shipments" ON public.shipment_events;
...
CREATE POLICY "Drivers can manage own PODs" ON public.shipment_pods FOR ALL USING (
    (
        select auth.uid()
    ) = driver_id
);
